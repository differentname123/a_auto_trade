...
def get_model_report_old(abs_name, model_name, need_reverse=False):
    """
    为单个模型生成报告，并更新模型报告文件
...
def process_abs_threshold(data, y_pred_proba, y_test, total_samples, abs_threshold, tree_threshold, thread_ratio, this_key, temp_dict_result):
    ...
    temp_dict = create_temp_dict(tree_threshold, 0, abs_threshold, unique_dates, precision, predicted_true_samples, total_samples, unique_dates_false, precision_false, predicted_false_samples_false, thread_ratio)
    if this_key not in temp_dict_result:
        temp_dict_result[this_key] = []
    temp_dict_result[this_key].append(temp_dict)
...
def process_cha_zhi_threshold(data, y_pred_proba, y_test, total_samples, cha_zhi_threshold, tree_threshold, thread_ratio, this_key, temp_dict_result):
    ...
    temp_dict = create_temp_dict(tree_threshold, cha_zhi_threshold, 0, unique_dates_diff, precision_diff, predicted_true_samples_diff, total_samples, unique_dates_diff_neg, precision_false, predicted_false_samples_diff_neg, thread_ratio)
    if this_key not in temp_dict_result:
        temp_dict_result[this_key] = []
    temp_dict_result[this_key].append(temp_dict)
...
def create_temp_dict(tree_threshold, cha_zhi_threshold, abs_threshold, unique_dates, precision, predicted_true_samples, total_samples, unique_dates_false, precision_false, predicted_false_samples_false, thread_ratio):
    temp_dict = {}
    temp_dict['tree_threshold'] = float(tree_threshold)
    temp_dict['cha_zhi_threshold'] = float(cha_zhi_threshold)
