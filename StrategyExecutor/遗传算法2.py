...
def select_parents(population, fitnesses):
    parents = np.random.choice(population, size=2, p=fitnesses / fitnesses.sum(), replace=False)
    ...
    for generation in range(generations):
        ...
        if fitnesses[max_fitness_index] > best_fitness:
            ...
            data['Weighted_Buy_Signal'] = (data['收盘'] - data['SMA']) * best_individual[0] + (data['收盘'] - data['EMA']) * \
                                          best_individual[1] + \
                                          data['MACD_Hist'] * best_individual[2] + (data['RSI'] - 30) * best_individual[
                                              3] + \
        ...
        if no_improvement_streak > 5:
            mutation_rate = min(0.5, mutation_rate + 0.02)  # 无改进时增加变异率
        ...
        # 随机重初始化
        num_random = population_size // 5  # 重新初始化种群的20%
...
elite_size = 10  # 精英策略大小

best_solution = genetic_algorithm(data, generations, population_size, crossover_rate, mutation_rate, elite_size)
print(best_solution)
